...
class Flyrobu:
      ...
      try:
        ...
        if title_element:
          ...
          price_element = price_new_element or result.select_one(
            ".price-normal")
          price = (price_element.text.strip().replace("₹", "Rs. ")
                   if price_element else None)

          brand_name = (result.select_one(".stat-1 a").text.strip()
                        if result.select_one(".stat-1 a") else None)
          image = (result.select_one(".image img")["src"]
                   if result.select_one(".image img") else None)
          description = (result.select_one(".description").text.strip()
                         if result.select_one(".description") else None)
          ...
          model_id = (
            ...
            if result.select_one(".stat-2 span:nth-of-type(2)") else None)
  ...
  def search(self, keyword):
    ...
    while True:
      try:
        ...
        next_page = soup.find("ul", class_="pagination").find("a",
                                                              class_="next")
        if next_page:
          url = next_page["href"]
        else:
  ...
  def get_product_details(self, product_name):
    """
        ...
        :param product_name: The name of the product to fetch details for.
        :type product_name: str
        :return: A JSON object containing the detailed product information.
        :rtype: dict or None
    ...
    try:
      ...
      description = soup.find(
        "div", class_="product_extra-242").find("div").text.strip()
      features_list = soup.find(
        "div", class_="product_extra-242").find("ul").find_all("li")
      ...
      if technical_details_table:
        ...
        for row in rows[1:]:
          attribute_name = row.find("td",
                                    class_="attribute_name").b.text.strip()
          attribute_value = row.find("td",
                                     class_="attribute_value").p.text.strip()
      ...
      current_price_elem = price_section.find("div",
                                              class_="product-price-new")
      original_price_elem = price_section.find("div",
      ...
      if current_price_elem is None and original_price_elem is None:
        ...
        ) if original_price_elem else None
