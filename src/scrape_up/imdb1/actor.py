...
class Actor:
    ...
    def popular_movies(self):
        ...
        try:
            name = self.page_soup.find_all("a", {"class" :"ipc-primary-image-list-card__title"})
            lis = []
            for n in name:
    ...
    def all_movies(self):
        ...
        try:
            all_movies = self.page_soup.find_all("a", {"class" : "ipc-metadata-list-summary-item__t"})
            lis = []
            for a in all_movies:
    ...
    def awards(self):
        ...
        lis = []
        try:
            awards = self.page_soup.find_all("a", {"class" : "ipc-metadata-list-item__label ipc-metadata-list-item__label--link"})
            lis.append(awards[1].get_text())
        except:
            pass

        try:
            awards2 = self.page_soup.find("span", {"class": "ipc-metadata-list-item__list-content-item"})
...
print(actor.popular_movies())
print(actor.all_movies())
print(actor.awards())
