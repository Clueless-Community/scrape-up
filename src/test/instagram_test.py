...
class InstagramWebScrapingTest(unittest.TestCase):
    ...
    def test_followers_success(self):
        ...
        mocked_data = f'<meta name="description" content="{expected_followers}, 200 followers">'
        with patch("requests.get") as mock_get:
            ...
            self.assertEqual(result["message"], f"Followers found for user {self.username}")
    ...
    def test_followers_index_error(self):
        ...
        with patch("requests.get") as mock_get:
            ...
            self.assertEqual(result["message"], f"Failed to retrieve followers count for user {self.username}")
    ...
    def test_followers_http_error(self):
        ...
        with patch("requests.get") as mock_get:
            ...
            result = self.user.followers()
            self.assertIsNone(result["data"])
            self.assertEqual(result["message"], f"Failed to retrieve page for user {self.username}. Error: {expected_error}")
